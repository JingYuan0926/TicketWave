...
const YourComponent = () => {
  const [nfts, setNFTs] = useState([]); // State to hold user's NFTs
    ...
    const fetchNFTs = async () => {
      ...
      if (window.ethereum) {
        try {
          ...
          const tokenContractAddress = '0x32697dC6d02dFB5DAA68fa8fFcD0a362b93fF96a';

          // ERC-721 ABI (Contract Interface)
          const  tokenABI = [
            {
              "inputs": [
                {
          ...
          setNFTs(tokens); // Set the NFTs in the state
        } catch (error) {
      ...
      <div>
        {nfts.map((tokenId, index) => (
          <div key={index}>
            ...
            {/* Add code to display additional NFT information */}
          </div>
        ))}
      </div>
    </div>
